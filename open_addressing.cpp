 ...
 *    1. Cache friendly. The inner loop always checks 4 items that are (usually) on the same cache line.
 *    2. Can deal with bad/trivial hash functions. Eventually, all bits of the computed hash will have an impact.
 *       This is because the variable perturb is shifted and mixed into the hash. Therefore, clustering as known
 *       from linear probing should not happen.
 *    3. Hits every slot in the table. This is important to guarantee correctness. Basically, when there is an
 *       empty slot it will be found. This is achieved using the hash * 5 + 1 part. The perturb will be 0 eventually.
 *    4. The first few collisions (the common case) are handled very cheaply.
 *    5. Naturally supports values being grouped together to avoid padding when they are interleaved.
 */
...
uint32_t prime_numbers[] = { 1, 3, 7, 13, 31, 61, 127, 251, 509, 1021,
...
536870912, 1073741789, 2147483647, 4294967291 };
